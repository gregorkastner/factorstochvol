...
RcppExport SEXP sampler(const SEXP y_in, const SEXP draws_in,
  ...
  arma::mat armaht(arma::size(armah));
  ...
  NumericMatrix curpara(4, mpr);
  for (int i = 0; i < m; i++) {
    curpara(0,i) = startmu(i);
    curpara(1,i) = startphi(i);
    curpara(2,i) = startsigma(i);
    curpara(3,i) = 0.;
    armaht.col(i) = (armah.col(i)-curpara(0,i))/curpara(2,i);
  }
  ...
  const double Bmu    = as<double>(Bmu_in);
  const arma::vec priormu = {bmu, Bmu};

  const NumericVector priorphi(priorphi_in);
  const arma::vec priorphi_idi = {priorphi(0), priorphi(1)};
  const arma::vec priorphi_fac = {priorphi(2), priorphi(3)};
  ...
  NumericVector para(4 * mpr * (draws/thin));
  para.attr("dim") = Dimension(4, mpr, draws/thin) ;
