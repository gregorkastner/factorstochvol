...
However, due to the large number of latent quantities, their efficient estimation is non-trivial and software that allows to easily fit SV models to data is rare.
  ...
  As opposed to previous versions, \stochvol{} is now capable of handling linear mean models, conditionally heavy tails, and the leverage effect in combination with SV.
...
\Address{
  ...
  URL: \url{https://statmath.wu.ac.at/~hosszejni/}
  \\\\
  Gregor Kastner\\
  ...
  URL: \url{https://www.aau.at/team/kastner-gregor/}
}
...
\section*{Preface}
This vignette corresponds to \citet{thisjss} published in the Journal of Statistical Software.
The version at hand might receive minor updates as time goes by.
Further information about citing \pkg{factorstochvol} can be obtained in \proglang{R} by installing the package, e.g., through \code{install.packages("factorstochvol")}, and calling \code{citation("factorstochvol")}.
...
The default value for $\bm\beta$ is the ordinary least squares estimator $(\bm{X}^\top \bm{X})^{-1}\bm{X}^\top\yvec$, where $\bm X$ denotes the regression design matrix and $\yvec$ denotes the vector of observations.
...
\begin{itemize}
  ...
  \item \code{thin} [\code{1}]: the amount of thinning (every \code{thin}th draw is kept);
...
\begin{figure}[tp]
  \centering
  \includegraphics[width=\textwidth, clip, trim=0 174 0 174]{jss3813-corimageplot-1}
...
Source code is available in the GitHub repositories \url{https://github.com/gregorkastner/stochvol} and \url{https://github.com/gregorkastner/factorstochvol}.
